...
MOCK_USERS_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'mock_users.json')
...
def register():
    ...
    try:
        ...
        # Graceful fallback to mock registration
...
def login():
    ...
    try:
        ...
        # Graceful fallback to mock login
        mock_users = load_mock_users()
...
def profile():
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT Customer_ID, Name, Email, Phone, Address, Created_At FROM Customers WHERE Customer_ID = %s", (current_user_id,))
        user = cur.fetchone()
        cur.close()
        ...
        # Fallback to mock profile retrieval
        try:
        ...
        mock_users = load_mock_users()
        user = next((u for u in mock_users if u['Customer_ID'] == user_id_int), None)
        if user:
            user_response = user.copy()
            user_response.pop('Password')
